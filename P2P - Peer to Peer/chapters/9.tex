...
\framedt{Forks and double spending}{
   ...
   But it may happen that the two transactions are mined simultaneously in two different blocks, resulting in a \textbf{fork}. To avoid so, \ul{a transaction is accepted by the network only after 6 blocks have been mined on top of it}.
}
