...
\note{The pigeonhole principle states that if $n$ pigeons are put into $m$ pigeonholes, with $n > m$, then at least one pigeonhole must contain two or more pigeons.}
...
\framedt{Birthday Paradox}{
   A hash function $H$ has $n$ possible outputs, so $2^n$ possible inputs.\\
