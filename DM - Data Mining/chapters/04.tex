...
\begin{itemize}
	...
	Especially for the first iterations when clusters are small, this can lead to significant space usage.
	}
...
\labelitemize{DBSCAN}{
	\begin{enumerate}
		\item Label points as core, border, or noise based on thresholds $R$ (radius of neighborhood) and \lstinline|min_pts| (min number of neighbors)
		\begin{itemize}
			\item Every point having at least \lstinline|min_pts| points within distance $R$ becomes a core point
		\end{itemize}
		\item Connect core points that are within $R$ of each other, hence are neighbors, putting them in the same cluster
