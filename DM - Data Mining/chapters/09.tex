...
of elements (transactions). Each element is a set of events (items) that occur together. A sequential
...
It may occur that there are multiple ways in which a subsequence can be contained in a data sequence. Each such way is called an \textbf{instance} or \textbf{occurrence} of the subsequence in the data sequence.
In sequential pattern mining, it's sufficient to find only one instance.
...
Note that a $k$-sequence \ul{contains} $k$ \textbf{events}, \ul{not necessarily }$k$ \ul{\textbf{elements}}!
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                ...
                However, the sequence must be exactly the same in terms of \textit{items}, not only \textit{events}!
                The following case would not be allowed:
            ...
            \begin{itemize}
                ...
                \[\langle\{a,b\}\{a,b\}\rangle + \langle\{a,b\}\{a,b\}\rangle = \textit{!!!}\]
            \end{itemize}
        \end{itemize}
    \end{itemize}
\end{itemize}
...
\note{No need to evaluate $(k-2)$-subsequences, because they have already been checked at the previous step! If one of them was not frequent, one of the $(k-1)$-subsequences containing it would also be infrequent, and we would have already pruned $W$ at that step.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics{images/09/candidatePruning.png}
    \caption{Example of candidate Generation and Pruning}
    \label{fig:09/candidatePruning}
\end{figure}
To understand how this works, consider the first generated candidate in \autoref{fig:09/candidatePruning}: $\langle\{1\} \{2\} \{3\} \{4\}\rangle$
\begin{itemize}
    \item We generate the following $(k-1)$-subsequences:
    \[\langle\{2\} \{3\} \{4\}\rangle, \langle\{1\} \{3\} \{4\}\rangle, \langle\{1\} \{2\} \{4\}\rangle, \langle\{1\} \{2\} \{3\}\rangle\]
    
    \item The first and the last subsequences are the ``mother'' ones, so we only need to check the other two
    \item They do not appear in the list of frequent 3-sequences, so we can prune this candidate
\end{itemize}
...
In the algorithm when we check whether a subsequence is frequent, do we have also to check whether it satisfies the given timing constraints, or is it sufficient to check only the support?
The answer depends on the type of timing constraint:
...
% Therefore, if a sequence does not satisfy the max span constraint, none of its super-sequences will satisfy it.
    ...
    We need to use \textbf{contiguous subsequences} to ensure that the max-gap constraint is preserved when generating candidates and counting support.
