...
The act of sliding a window through a function is called \textbf{convolution}.
...
\begin{itemize}
   ...
   \end{align*}
   \note{$s_{t:} $ indicates the series from time $t$ to the end, while $s_{:t+\Delta:}$ indicates the series from the start to time $t+\Delta$.}
...
\end{itemize}
\note{Descriptive \textit{power} and discriminative \textit{power}, respectively.}
...
\begin{itemize}
   ...
   \[c^a_{i,i} = ||l_i - u_j||_p\]
   \item \textbf{Match cost} - cost of foregoing matching $i$ with $j$ in favor of a lower cost $ i' \neq i$ (i.e. ``How much would I pay for another alignment having a smaller cost?'')
\end{itemize}
\note{The latter is useful when considering warping, because it allows us to skip components in favor of a lower cost alignment.}
