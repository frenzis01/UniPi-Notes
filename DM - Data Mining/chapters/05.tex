...
The closer the correlation is to $-1$, the better the clustering, because, points which are far apart and are in different clusters have inversely correlated values (high distance, low ---0 actually--- similarity), while points which are close together and are in the same cluster have also inversely correlated values (low distance, high similarity).
...
\begin{paracol}{2}
	...
	\begin{itemize}
		...
		\begin{enumerate}
         \item[$\circ$] Suppose there are clusters $A, B, C, D$ and $i$ is in cluster $A$
         \item Calculate average distance of $i$ to points in $B$, call it $d_B$, do the same for $C$ and $D$, obtaining $d_C$ and $d_D$
         \item Then $b = \min(d_B, d_C, d_D)$ 
      \end{enumerate}}
...
\subsection{Statistics to evaluate metrics}
\coolquote{
   Suppose you get a 0.6 silhouette score for a clustering. Is that good or bad?
}{Data miner}

To answer this question, we can use \textbf{statistical} hypothesis testing. The idea is to compare the observed silhouette score to a distribution of silhouette scores obtained on random data with no inherent cluster structure.\\
...
\begin{figure}[htbp]
   \centering
   \includegraphics{images/05/statisticsSSE.png}
   \caption{Histogram shows SSE of three clusters in 500 sets of random data points of size 100 distributed over the range $[0.2, 0.8]$ for $x$ and $y$ values.
   The clustering on the left has SSE of 0.005, which is significantly lower than the random data ---in fact it is out of scale---, indicating a good clustering.}
   \label{fig:05/statisticsSSE}
\end{figure}

Instead if we have two clustering results, one yielding 0.6 silhouette score and another yielding 0.8 silhouette score, we can easily conclude that the second clustering is better, since it has a higher silhouette score.
We do not want to assert whether the value is \textit{good} in an ``absolute'' way, but rather whether it is \textit{better} than another clustering result.