...
Shapelets can provide interpretable results,which may help domain practitioners to better understand their data.\\
...
\begin{algorithmic}[1]
   ...
   \State $\textit{bsf}\_gain \gets 0$
   \State $\textit{bsf}\_shapelet \gets \text{null}$
   \For{each shapelet $S$ in $candidates$}
      \State $gain \gets \text{CheckCandidate}(D, S)$
      \If{$gain > \textit{bsf}\_gain$}
         \State $\textit{bsf}\_gain \gets gain$
         \State $\textit{bsf}\_shapelet \gets S$
      \EndIf
   \EndFor
   \State \Return $\textit{bsf}\_shapelet$
\end{algorithmic}
...
The problem is that we have many candidates, and for each candidate we need to compute the distance from each time series sample to the candidate in order to compute the information gain, which is expensive.
\[
...
\begin{enumerate}
   ...
   \begin{paracol}{2}
      ...
      The information gain upper bound can be computed by assuming that all the remaining samples are split in the best possible way, i.e., all the remaining samples of the same class as the majority class in $D_1$ are assigned to $D_1$, and all the remaining samples of the same class as the majority class in $D_2$ are assigned to $D_2$.
      \colfill
   \end{paracol}
