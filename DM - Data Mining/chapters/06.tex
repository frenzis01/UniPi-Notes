...
Grubbs' test iterates over detected outliers, \ul{removing \textit{one layer} of outliers at a time, until no more outliers are found}.
...
\begin{table}[htbp]
   \begin{tabular}{|c||c|}
      ...
      \hline
      \textbf{Sensitive} & Outliers influence the distribution but may be removed by Grubbs' test \\
      \hline
...
\begin{table}
   \begin{tabular}{|c||c|}
      ...
      \hline
      \textbf{Sensitive} & Outliers influence the distribution, may be unstable, but may be removed by Grubbs' test (?) \\
      \hline
      \textbf{Interpretable} & \textbf{Black-box}, no clear explanation for outlierness, simply, there are not many similar instances.\\
      \hline
   \end{tabular}
...
\framedt{Grubbs' test}{
   Choosing a threshold $\hat{o}$ is arbitrary, but there are algorithms such as Grubbs' test which define their own thresholding mechanism.
   In Grubbs' test, the threshold is defined as:
   \[
...
Manifold-based algorithms are as flexible as the defined manifold. Like with mixture models, neighbor-based approaches reintroduce locality, so outliers are defined in function of their neighbors:
\begin{itemize}
...
\begin{paracol}{2}
   ...
   \colfill
   Connectivity and concentration can be approximated\\ through similar structures: we go from \textit{postings} matrix to distance matrix!
...
Note that this is different from picking ${x_j \in B_i}$, as $B_i$ considers $\delta$ rather than $\varepsilon$: ${x_j \in \mathcal{X} : d(x_i,x_j) \leq \delta}$.

{that is, two-radii concentration compares the concentration of an instance, with the
concentration of its neighbors. For\ns
\begin{itemize}
   \item $\tilde{o}(x_i) > 0$ neighbors have a higher concentration. $x_i$ is in a sparse area, it could be an outlier!
   \item $\tilde{o}(x_i) \approx 0$ neighbors have a the same concentration 
   \item $\tilde{o}(x_i) < 0$ neighbors have a lower concentration. $x_i$ is in a dense area, likely not an outlier!
\end{itemize}
}
...
\begin{paracol}{2}
   ...
   In the typical isolation trees the splits are univariate, that is, they split the space along one attribute at a time, so it follows a hyperplane orthogonal to the axis of the chosen attribute (see \autoref{fig:06/hyperplanes}).
   However in EIF (Extended Isolation Forests) splits can be multivariate, that is, they can split the space along hyperplanes not orthogonal to any axis, as they are displayed here
   \begin{itemize}
   ...
   That is, the instance is an outlier it the distance from the root is \textbf{lower} than average.
   
   Note that, differently from other approaches, here \ul{\textit{lower} outlier degree means \textit{higher} outlierness}.
   \colfill
   ...
   \begin{figure}[htbp]
      \centering
      \includegraphics[width=0.98\columnwidth]{images/06/isoltree.png}
...
\ul{Dense data would end up in a super deep leaf}, potentially making less dense but still normal instances to be isolated \textit{``relatively''} quickly, and thus misclassified as outliers.
That's the reason why we set a maximum height for the tree, to have a cutoff point after which we stop splitting the space and we consider the instances in the leaf as normal.
Furthermore, we avoid unnecessary computations, as instances that are very deep in the tree are very likely not outliers.
