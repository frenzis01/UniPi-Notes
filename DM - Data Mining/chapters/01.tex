...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=0.9\columnwidth]{images/01/knowledgeDiscoveryLoop.png}
...
\node[box, right=1.5cm of preproc] (mining) {Data \\ Mining};
\node[box, right=1.5cm of mining] (postproc) {Postprocessing};
\node[textnode, right=1.5cm of postproc] (info) {Information};
...
{Attribute types:\ns
\begin{itemize}
   \item \textbf{Nominal/Categorical}: attribute values in a finite domain (\textit{distinctness})
   \item \textbf{Binary}: special case of nominal with two values
   \item \textbf{Ordinal}: attribute values have a total ordering (\textit{distinctness} and \textit{order})
   \item \textbf{Numeric}: quantity (integer or real-valued) (\textit{distinctness}, \textit{order}, \textit{differences})
   \item \textbf{Ratio-Scaled}: we can speak of values as being an order of magnitude larger than the unit of measurement (\textit{all 4 properties})
...
\begin{paracol}{2}
   ...
   \begin{figure}[htbp]
      \centering
      \includegraphics[width=0.95\columnwidth]{images/01/semantics.png}
...
\begin{itemize}
   ...
   \item \textbf{Spearman correlation}
         ...
         \note{\textit{rank} is the position of the value in the sorted list of values.}
\end{itemize}
...
Quantiles and distributions inform us on what values may be outliers. 
