...
Assuming a minimum support threshold of 2, let's consider which are the \textbf{maximal frequent itemsets} from the example:
\begin{itemize}
...
where:
\begin{itemize}
   \item $\sigma(X \cup Y)$ is the support of the itemset that contains both $X$ and $Y$.
   \item $\sigma(X)$ is the support of the itemset $X$.
\end{itemize}
...
% \[\text{lift}(X \Rightarrow Y) = \frac{P(X,Y)}{P(X) \cdot P(Y)}\]
\[\text{lift}(X \Rightarrow Y) = \frac{P(Y|X)}{P(Y)} = \frac{P(X,Y)}{P(X)\cdot P(Y)}\]
...
\subsection{Interest}
The \textbf{interest} of an itemset $X$ is a measure of how much the actual support of $X$ deviates from what would be expected if the items in $X$ were independent. 
\ul{It is essentially the same as lift, but defined for itemsets rather than rules.}
It is defined as:

\[\text{interest}(X) = \frac{P(X,Y)}{P(X) \cdot P(Y)}\]
...
\note{The first, \textit{PS} may stand for \textit{``probability surplus''} (name in probabilistic/statistical context), which in the Association Rule Mining context corresponds to \textit{``Leverage''}, but I'm not sure.}

\section{Non-binary Attributes}

The measures we just saw are defined for binary attributes (itemsets), but in real-world datasets we often encounter non-binary attributes, which can be categorical or continuous.\\
Hence, how can we apply association rule mining to non-binary attributes?
...
\]
where, aside from $X$ and $Y$ being the set of instances and labels respectively, we have:
\[
...
\]
\[
l_0(R,X,Y)=\frac{1}{n}\sum_{(x,y)\in X\times Y}\Big(
\operatorname{supp}(y_R^0,x)\Big)\;
\mathbf{1}[y\neq y_R^{0}]\qquad \text{alternative definition}
\]

To read the formula, the fraction and the sum over $X \times Y$ is just the average over all instances, which will yield a ratio.
...
$\lambda k^R$ accounts for the complexity of the rule list, with $\lambda$ controlling how much we want to penalize longer lists.
Being $l_R(R,X,Y)$ the empirical error due to the rules in the list, 
it is smaller than $L(R,X,Y)$, which also includes the error due to the default rule.\\
That is, the lower bound $b(R,X,Y)$ is a lower bound on the total loss $L(R,X,Y)$, because it only accounts for the error due to the rules, and not for the error due to the default rule.

The overall idea, if I understood correctly is that extensions $R^+$ of a rule list $R$ will always have a loss $L_R(R^+)$ greater than or equal to the loss of the parent rule list $L_R(R)$, because adding more rules can only increase the number of misclassifications (or leave it unchanged, because the rules in $R$ have priority, and if they were picked before, they will still be picked even if there are more rules ``at the bottom'').
We can only decrease $L_0(R)$, the loss due to the default rule.
Thus, if we have already found a rule list $R_{best}$ with a certain loss lower than $R$'s, we can prune any extensions of $R$ that cannot improve upon this loss. 
...
\begin{figure}[htbp]
   ...
   \caption{Each transaction is a path in the trie. There is a header table to point to the first occurrence (left-tree-traversal) of each item in the tree.
   Each item has pointers towards the same item.}
   \label{fig:08/trie1}
\end{figure}
...
of the data
\begin{enumerate}
	\item \textit{Extreme case A}: All transactions are the same, the FP-tree is a single branch
	\item \textit{Extreme case B}: All transactions are different the size of the tree is the same as that of the database (bigger actually since we need additional pointers)
\end{enumerate}
...
We want, given an FP-tree, to find all frequent itemsets and their support.
The method we can exploit follows the divide-and-conquer paradigm, and it is called \textbf{FP-Growth}.
The intuition is the following: consider all itemsets that end with a given suffix $X$, for instance $EDCBA$.
\begin{itemize}
   \item For each possible ending item, consider the itemsets with the last item equal to one of items preceding it in the ordering
   \item e.g. for E, consider all itemsets with last item $D,C,B$ or $A$. In this way we get all the itemsets ending at $DE, CE, BE, AE$.
   \item Proceed recursively in this way.
   \item Iterate for all the items.
\end{itemize}

\begin{figure}[htbp]
   \centering
   \includegraphics{images/08/frequentItemsets.png}
   \caption{This represents the search space of possible solutions. As you can see A has no children, because it's the first item in the ordering, so there may be no items preceding it. This contains all the suffixes we will consider at each iteration of the algorithm. There may be some suffixes which do not occur in the transactions, but we will consider them anyway, and then we will find out that they are not frequent.}
   \label{fig:08/frequentItemsets}
\end{figure}
...
\begin{algorithmic}[1]
\State $\bar{X}$ = set of possible suffixes in the FP-tree \footnote{Get these following pointers starting from header table}
\For{each suffix $X$}
...
\begin{figure}[htbp]
   \centering
   \includegraphics{images/08/trie2.png}
   \caption{Final trie with all transactions and pointers. Pointers are used to assist frequent itemset generation}
   \label{fig:08/trie2}
\end{figure}

\begin{figure}[htbp]
   \centering
   \includegraphics[width=0.45\columnwidth]{images/08/trie3.png}
   \includegraphics[width=0.3\columnwidth]{images/08/trie4.png}
   \caption{We may get all the possible suffixes by following the pointers from the header table and then traversing the tree bottom-up from each pointed node}
   \label{fig:08/trie3}
\end{figure}
...
\begin{paracol}{2}
   \colfill
   \paragraph*{Recomputing support}:
   The support of a node becomes the sum of the support of the leaves with label E in its subtree.
   \colfill
   \switchcolumn

   \begin{figure}[htbp]
      \centering
      \includegraphics[width=0.7\columnwidth]{images/08/trie5.png}
      \caption{Recomputed the support of the nodes, we remove the nodes with $E$}
      \label{fig:08/trie5}
   \end{figure}
\end{paracol}
...
\begin{paracol}{2}
   ...
   \paragraph*{Pruning infrequent items}:
   We remove all nodes with support less than minsup.
   \colfill
   ...
   \begin{figure}[htbp]
      \centering
      \includegraphics[width=0.7\columnwidth]{images/08/trie6.png}
      \caption{Pruned $B$ (minsupp = 2)}
      \label{fig:08/trie6}
   \end{figure}
\end{paracol}
...
\begin{paracol}{2}
   \colfill
   \paragraph*{Repeat recuversively}:
   At this point we have to repeat the process for ${DE},{CE},{AE}$.
   We start off with ${DE}$, removing nodes without $D$ as suffix.
   \note{$AE$ does not appear in the dataset, but we will consider it anyway because it is present in the search space in \autoref{fig:08/frequentItemsets}, and then we will find out that it is not frequent.}
   \colfill
   ...
   \begin{figure}[htbp]
      \centering
      \includegraphics{images/08/trie7.png}
      \caption{Removing $C$, as there is no $D$ in its subtree}
      \label{fig:08/trie7}
   \end{figure}
...
\begin{paracol}{2}
   \colfill
   \paragraph*{Recomputing support and Pruning}:
   Support count does not vary here, but we do have to prune infrequent items.

   Being left with only $A$, which has support 2, we know that ${ADE}$ is frequent with support 2, and since there are no more nodes, we move onto the next subproblem, i.e., ${CE}$.
   \colfill
   ...
   \begin{figure}[htbp]
      \centering
      \includegraphics{images/08/trie8.png}
      \caption{Pruning nodes $D$ at the bottom, and then $C$.}
      \label{fig:08/trie8}
   \end{figure}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=0.45\columnwidth]{images/08/trie9.png}
   \includegraphics[width=0.45\columnwidth]{images/08/trie10.png}
   \caption{Get paths ending with $C$, then recompute support and prune nodes. All nodes are pruned, so no itemset with $CE$ as suffix is frequent}
   \label{fig:08/trie9}
\end{figure}

Doing the same for $A$ yields that ${AE}$ is frequent with support 2.
In the end, we have the following frequent itemsets ending with $E$:
\[E, DE, CE, AE, ADE\]

At this point we must proceed iterating the whole process with D, then C, B, and A.

Note that all the subproblems solved at each iteration are \textbf{disjoint} so we never consider the same itemset twice.