...
and the number of classes is known.

There are two main approaches: \textbf{entropy} and \textbf{percentiles} based discretization.
...
The splitting process goes on with another interval, typically choosing the interval with the worst (highest) entropy.
...
\begin{itemize}
	...
	\item In statistics, \textbf{standardization} refers to subtracting off the mean and dividing by the standard deviation
\end{itemize}
...
\end{itemize}
\item Square Root and Reciprocal (- Stabilizing Variance) 
\[T(x) = ax^p + b\]
\begin{itemize}
...
To detect outliers on one dimension we can use also the \textbf{IQR} (Interquartile Range) method or the \textbf{Z-Score} method.
...
\begin{itemize}
	\item \textbf{IQR} method:
	      \begin{itemize}
		      ...
		      \item Define lower bound = Q1 - 1.5 * IQR
		      \item Define upper bound = Q3 + 1.5 * IQR
		      \item Any data point outside these bounds is considered an outlier
	      \end{itemize}
	\item \textbf{Z-Score} method:
	      \begin{itemize}
		      \item Calculate the mean ($\mu$) and standard deviation ($\sigma$) of the dataset
		      \item For each data point $x$, compute the Z-score: $Z = \frac{(x - \mu)}{\sigma}$
		      \item A common threshold is $|Z| > 3.29$; data points with Z-scores beyond this threshold are considered outliers
	      \end{itemize}
\end{itemize}