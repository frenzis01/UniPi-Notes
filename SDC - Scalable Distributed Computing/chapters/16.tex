...
\ul{The \textbf{offset} is a unique identifier for each message within a partition, and is used by consumers to keep track of their progress. The offset is always increased, and is never reset.}
...
\subsubsection{Rebalancing (Partitions number)}
...
\note{Consumer group rebalancing (discussed later in Section on Rebalancing) is different and more common: it reassigns existing partitions among available consumers when members join or leave the group, without changing the number of partitions. This happens automatically and frequently in production systems.}
...
\begin{paracol}{2}
   \begin{figure}[htbp]
      \centering
      \includegraphics[width=0.95\columnwidth]{images/16/singlecluster.png}
      \caption{Kafka brokers cluster}
      \label{fig:16/singlecluster}
   \end{figure}
...
\begin{paracol}{2}
   ...
   \begin{figure}[htbp]
      \centering
      \includegraphics[width=0.95\columnwidth]{images/16/producerRecord.png}
