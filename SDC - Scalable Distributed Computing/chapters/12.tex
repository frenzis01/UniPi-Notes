...
\begin{definition}
   [Read Skew]
   A \textbf{read skew} occurs when a transaction reads different temporal versions of the same data, leading to a \textit{temporary inconsistency}, violating an invariant.
\end{definition}

Read skews are possible in \textit{Read Committed} isolation level, 
\begin{figure}[htbp]
   \centering
...
\begin{paracol}{2}
   \begin{figure}[htbp]
      \centering
      \includegraphics[width=0.95\columnwidth]{images/12/mvcc.png}
      \caption{Snapshot isolation and MVCC}
      \label{fig:12/mvcc}
   \end{figure}
