...
\labelitemize{Strategies}{
   ...
   Conflict resolution may happen \textit{on write}, \textit{on read}, or at \textit{row/document} level.
}
...
Allows systems to operate offline, useful for applications needing to function without network connection.
Reduces latency by placing replicas closer to users, enhancing user experience.

Besides, it scales read operations by distributing the load across multiple replicas.
Essential for handling large volumes of read requests efficiently.
Managing concurrent writes and ensuring data consistency across replicas can be challenging.

Different replication strategies offer various trade-offs in terms of consistency, availability, and performance.
Understanding consistency models like eventual consistency and strong consistency is crucial.
Effective strategies for fault tolerance are necessary to maintain system integrity.