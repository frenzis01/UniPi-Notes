...
\begin{enumerate}
	\item Closure $P$ is closed under the execution of $S$: that is, once $P$ is established in $S$, it cannot be falsified, it can't go in an illegitimate state.
...
\begin{itemize}
   \item A system is said to be randomized selfstabilizing system, if and only if it is selfstabilizing and the expected number of rounds needed to reach a correct state (legal state) is bounded by some constant k.
