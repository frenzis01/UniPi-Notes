...
\begin{itemize}
   ...
   Paxos works by ensuring a majority of nodes agree on a value.
   \item Leader-based coordination:
   Sometimes protocols use a leader to drive agreement, such as in Raft.
...
\textbf{Safety} ensures that the systems never reaches an incorrect state. In the context of consensus, safety means that if a value is decided, it must be the same across all nodes.\\
...
It can be thought as the ``something good eventually happens''.
...
\subsubsection{Why the Promise Check is Essential}
The promise mechanism in Phase 2b guarantees \textbf{safety} by preventing older proposals from overriding newer ones.

When an \textsc{Acceptor} sends a \textsc{Promise(n)} in Phase 1b, it commits to ignoring any future proposals with number $< n$.

\textbf{Example scenario without this check:}
\begin{enumerate}
   \item Proposer A sends \textsc{Prepare(10)} $\rightarrow$ Acceptor responds \textsc{Promise(10)}
   \item Proposer B sends \textsc{Prepare(20)} $\rightarrow$ Acceptor responds \textsc{Promise(20)} (more recent!)
   \item Proposer A sends \textsc{Accept(10, value\_A)}
   \begin{itemize}
      \item \textit{Without the check}: Acceptor would accept $value\_A$
      \item \textit{With the check}: Acceptor rejects because it already promised to only consider $n \geq 20$
   \end{itemize}
\end{enumerate}
