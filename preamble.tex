...
\newcommand{\altfrac}[2]{\ifmmode\def\tmp{$}\else\def\tmp{}\fi\mbox{%
    {\raisebox{.22\ht\strutbox}{\tmp#1 \tmp}}%
    \kern+0.1pt\scalebox{1.3}[1.3]{/}\kern+0.1pt%
    {\tmp #2\tmp}%
    }}

\newcommand{\textblue}[1]{{\color{blue}#1}}

% --- Caption settings: use "Fig." and colon as default separator
\captionsetup[figure]{name=Fig.,labelsep=colon} % default: "Fig. 16.3: caption..."

% --- Helper: caption that shows only the numbered prefix if argument empty
\makeatletter
\newcommand{\figcaption}[1]{%
  \begingroup
    % if the argument is empty -> set no separator and produce a numbered caption with empty body
    \if\relax\detokenize{#1}\relax
      \captionsetup{labelsep=none}% no ":" after the label
      \caption{ }% numbered caption (produces "Fig. <n>") with no text
    \else
      \caption{#1}% normal caption (produces "Fig. <n>: <text>")
    \fi
  \endgroup
}
\makeatother
...
\definecolor{verbBG}{RGB}{235, 231, 255}
\definecolor{verbText}{RGB}{24,0,156}

% Redefine \verb to use colorbox with custom styling
\makeatletter
\def\verb{\@ifnextchar|{\verb@colored}{\oldverb}}
\def\verb@colored|#1|{\colorbox{verbBG}{\footnotesize\ttfamily\color{verbText}\detokenize{#1}}}
\makeatother
