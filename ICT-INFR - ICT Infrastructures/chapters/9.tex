...
\begin{itemize}
   ...
   \note{Such as the one demonstrated by prof. Cisternino.}
\end{itemize}
...
\labelitemize{\textit{Authentication}}{
   \begin{itemize}
      \item OAuth
...
\labelitemize{\textit{Authorization}}{
   \begin{description}
      \item[] 
      \quad Unix systems enforce a \textit{default deny} policy, and users are added to groups to grant them permissions.\\
...
\framedt{``Accountability'' in GDPR}{
   ...
   In GDPR appears the curious term \textit{``accountable''}, i.e. being able to produce all the documentation that proves that you are compliant with the GDPR, meaning that you did everything you should have done to protect the data.\\
