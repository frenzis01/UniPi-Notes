...
\textbf{Passive-Active} means that aside from the active system, there is a mirrored one which is shut down waiting for failure and boots up \textit{``just in case''}.
This approach is usually not the ideal one, because the second system is very unlikely to be used and is costful.

\textbf{Active-active} systems are usually better, bust cost even more.
